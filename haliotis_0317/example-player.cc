	...
	**/
	for(int i=0; i<length; i++){
		...
		if(myBoard[start.row][start.column]== player) {
			...
			printf("Update %c%i to %c%i\n", myMarbleVector[it].horizontal, myMarbleVector[it].diagonal, 
											(*myMarblesAfterMove)[it].horizontal, (*myMarblesAfterMove)[it].diagonal);				
		}		
			...
			printf("Update %c%i : %c%i\n", 	oponentMarbleVector[it].horizontal, oponentMarbleVector[it].diagonal, 
											(*opMarblesAfterMove)[it].horizontal, (*opMarblesAfterMove)[it].diagonal);
		}
...
void evaluate_move(Move* m, vector<Marble>* myMarblesAfterMove, vector<Marble>* opMarblesAfterMove, board boardAfterMove)
{
	...
	if(changed_mean_dist_mine > 0){
		m->value += 0*changed_mean_dist_mine;
	} 
	...
	m->value += 0*(marbles_in_danger_before - marbles_in_danger_after);
